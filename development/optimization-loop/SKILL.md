...
> "Scan for: dead code, inconsistent error handling, type-safety gaps, hardcoded values that should be config, missing input validation at boundaries, unhandled rejections, AND over-engineering on live code: single-implementation interfaces / abstract classes, factories or wrappers with one product, layers with one caller, config keys read at one constant value, hand-rolled code the stdlib or platform already ships, and dependencies a few lines would replace. Report with file paths and line numbers; tag each over-engineering finding `delete` / `stdlib` / `native` / `yagni` / `shrink` and name what replaces it."
>
> **(FUGAZI)** If [FUGAZI](https://github.com/AP3X-Dev/FUGAZI) (`fugazi` / `fugazi-mcp`) is available, ground this agent in deterministic findings instead of eyeballing: `fugazi dead-code --format json`, `fugazi health --format json` (complexity), `fugazi dupes --format json`, `fugazi boundaries --format json`. Treat dead-code findings as *candidates to confirm* — reflection, DI, and dynamic import defeat static reachability, and a library's public API reads as "unused" internally, so it never gets auto-deleted (that's Phase 3's confirm-before-destroying rule). The `dupes` clusters and the complexity/maintainability scores are your over-engineering signal — a clone or a one-caller wrapper is a `shrink`/`yagni` candidate. Read-only only; never run `fugazi fix` in an audit.
...
| **Over-built, earns nothing** | Live code carrying speculative abstraction / needless indirection (one-impl interface, wrapper-for-one, config-for-a-constant, hand-rolled stdlib) | Medium — delete-candidate; confirm no reflection/DI/plugin caller first, and if it may be a deliberate public/extension seam, route to **improve-architecture** rather than auto-collapsing |
