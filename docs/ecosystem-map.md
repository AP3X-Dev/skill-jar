...
**Operation → runtime path.** [runtime-path-forensics](../development/runtime-path-forensics/SKILL.md)
reconstructs one event-time operation across candidate instances, artifacts,
attempts, correlation edges, clock intervals, and terminal effects. Missing
telemetry routes to instrument-observability; repair routes to diagnose-loop;
unknown change consumers route to change-impact-proof; launch judgment remains
with production-readiness.

**Handoff → continuation.** [handoff-recovery](../development/handoff-recovery/SKILL.md)
reconciles an interrupted task against every relevant worktree, local-change
custody, durable state source, and current check. It emits one read-only
continuation and stops; fragmented planning routes to plan-prune, while incident
repair routes to diagnose-loop.
...
|---|---|---|
| detection-only | reads + reports, writes no code | handoff-recovery, runtime-path-forensics, change-impact-proof, decision-evidence-ledger (unless an exact ledger write is requested), verification-evidence-plan, arch-drift-watch (L1), improve-architecture (explore), production-readiness, data-store-selection |
