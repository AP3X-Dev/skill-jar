...
| Needs a new skill added to this jar | [**add-to-jar**](development/add-to-jar/SKILL.md) | Adds exactly one dropped/imported skill, regenerates jar metadata, queues Skill Forge, and gates with audit. |
| Needs proof of which runtime path handled one operation | [**runtime-path-forensics**](development/runtime-path-forensics/SKILL.md) | Reconstructs event-time instance, artifact, attempt, correlation, clock, and effect evidence without promoting control-plane state or missing logs. |
| Needs to resume interrupted repository work safely | [**handoff-recovery**](development/handoff-recovery/SKILL.md) | Reconciles worktrees, local-change custody, durable state, claims, and current checks into one bounded continuation. |
| Needs an evidence-grounded explanation of one subsystem | [**subsystem-explanation**](development/subsystem-explanation/SKILL.md) | Traces composition, control/data flow, state, effects, variants, and proof limits without inventing runtime behavior. |
| Needs proof that rendered output matches a visual reference | [**visual-parity-proof**](development/visual-parity-proof/SKILL.md) | Compares identity-bound captures under frozen surfaces, rules, masks, and tolerances with weakest-surface verdicts. |
...
Drop-in skills are reconciled through [**add-to-jar**](development/add-to-jar/SKILL.md)
and `python scripts/sync-jar.py`, which updates generated indexes, plugin
manifests, Skill Forge queue state, and hook evidence state before the audit
gate runs.
...
Most skills do a domain job. A smaller set helps maintain and improve the jar
itself.

These skills are tagged with `meta` and may use `core: true` in frontmatter.
They are still installed through normal categories, but the metadata lets agents
and maintainers identify the self-improvement substrate.

For most forks, keep the core skills and add your own domain skills on top. That
preserves the jar's ability to audit, test, review, and improve its own contents
over time. Evidence will grow as real proof packets are added; see
[`docs/evidence-model.md`](docs/evidence-model.md),
[`docs/core-skills.md`](docs/core-skills.md), and
...
## Adding a skill to this jar

Use [**add-to-jar**](development/add-to-jar/SKILL.md) when adding or importing a
new skill folder here.

1. Put exactly one skill at `<category>/<skill-name>/SKILL.md`.
2. Run `python scripts/sync-jar.py`.
3. Review generated and state diffs: `skills.json`, plugin manifests,
   `docs/core-skills.md`, `agent-state/SKILL_FORGE_TRACKER.md`, and
   `agent-state/skill-usage.md` if it changes.
4. Run `python scripts/audit-jar.py`.
5. Commit the skill, generated files, and state together.
...
| Skill | What it does |
|-------|--------------|
| [**add-to-jar**](development/add-to-jar/SKILL.md) | Adds or imports exactly one skill into this repo, runs `sync-jar.py`, inspects the generated indexes/manifests and agent-state diffs, queues the skill for Skill Forge pressure testing, and gates the change with `audit-jar.py` before a local commit. |
...
| [**simplify-loop**](development/simplify-loop/SKILL.md) | A **behavior-identity-gated** loop that safely collapses *live* over-engineering — the reduce-live-complexity counterpart to dead-code-reaper. A Scout files only structurally-provable single-use mechanism (forwarding wrappers, single-caller indirection, config read at one constant, a factory-for-one with no public/DI/plugin binding); a Collapser writes a characterization test first, then inlines one cluster per cycle proving output **byte-identical**; an independent Validator re-checks the test wasn't loosened and a complexity/LOC ratchet fell. Possible seams route to improve-architecture, behavior changes to optimization-loop. Builds + dry-runs, then **offers** launch. |
...
| [**change-impact-proof**](development/change-impact-proof/SKILL.md) | Traces a proposed or completed change beyond its edited lines to every relevant reachable consumer, defines decisive safety assurances, and ties a bounded verdict to source, execution, or named-runtime evidence without modifying the codebase. |
| [**runtime-path-forensics**](development/runtime-path-forensics/SKILL.md) | Reconstructs which event-time route, instance, artifact/config, retry or delivery, and terminal effect handled one bounded operation. Emits typed evidence, candidates, attempts, correlation edges, timeline intervals, hops, negative-evidence quality, weakest-link verdicts, and the cheapest safe discriminator without mutating the system. |
| [**handoff-recovery**](development/handoff-recovery/SKILL.md) | Recovers the exact state of interrupted repository work by binding the request to a repository, worktree, revision, task, environment, and authority; preserving every local change; reconciling handoffs and durable state against current evidence; and returning one typed read-only continuation or a precise blocker. |
| [**subsystem-explanation**](development/subsystem-explanation/SKILL.md) | Explains one bounded subsystem from composition-root, source, wiring, test, and supplied runtime evidence. Emits a typed path, state/effect boundaries, variants, contradictions, unknowns, and proof-limited verdict without mutating the system. |
| [**visual-parity-proof**](development/visual-parity-proof/SKILL.md) | Proves or rejects rendered visual parity across bound pages, states, viewports, images, frames, or PDF pages. Freezes coverage and tolerances before comparison, keeps capture/transform/mask identity explicit, evaluates every material region, and derives an exact, tolerance, mismatch, or incomparable verdict without fixing the UI or approving release. |
| [**decision-evidence-ledger**](development/decision-evidence-ledger/SKILL.md) | Creates or reconciles a durable, append-only provenance ledger that keeps observations, assumptions, proposals, decisions, unknowns, contradictions, authorities, consumers, and next evidence explicitly typed and linked. Defaults to read-only analysis and writes only an explicitly named, authorized ledger artifact. |
| [**verification-evidence-plan**](development/verification-evidence-plan/SKILL.md) | Converts one exact engineering claim into an ordered, runnable, falsifiable evidence plan with stable traceability, explicit evidence floors, biting-path checks, freshness rules, stop conditions, and honest gaps without executing or scheduling the checks. |
| [**instrument-observability**](development/instrument-observability/SKILL.md) | Adds production-grade app observability with Sentry by default: specialist investigation first, centralized telemetry setup, privacy filtering, workflow spans/breadcrumbs, user/agent attribution, release/source-map checks, cost signals, Electron/worker boundaries, tests, smoke events, dashboards, and actionable alerts. |
...
| [**unit-test-quality**](development/unit-test-quality/SKILL.md) | Builds, audits, and repairs unit tests so they prove behavior instead of merely raising coverage: fast, isolated, deterministic, assertion-strong, mutation-resistant, and free of common AI slop test patterns. |
...
| [**spec-driven-change**](development/spec-driven-change/SKILL.md) | Spec-driven development as **self-contained, tracker-free markdown** — agree on WHAT before code and keep the spec current *by construction*. Each change is a folder (proposal + tasks + a typed spec **delta**) whose requirements use a machine-checkable `### Requirement:` / `#### Scenario:` GIVEN/WHEN/THEN grammar; a bundled stdlib validator gates it; on archive the delta merges (strict `RENAMED→REMOVED→MODIFIED→ADDED`) into a living `openspec/specs/` layer so requirements never drift. Hands a validated change folder to **autonomous-advisor** / **sprint-ticket-runner** in place of a hand-written PRP. No `openspec` CLI, no npm, no issue tracker — two optional Python-stdlib scripts stand in for validate/merge; absence degrades to the by-hand rules. |
...
| [**guardrail-forge**](development/guardrail-forge/SKILL.md) | Bootstraps a project-owned **architecture enforcement pack** for an established repository: read-only discovery separates observed, documented, verified, approved, and enforced rules; conflicts stop for a human decision; then explicit approval unlocks deterministic validators, exact baselines and exceptions, adversarial fixtures, hooks, and CI. A separate breaker and verifier pressure-test the pack before it hands continuing detection to `arch-drift-watch`. MemBerry may enrich discovery but is never required for enforcement. |
...
| [**rebuild-panel**](development/rebuild-panel/SKILL.md) | **First-principles rebuild analysis** for a subsystem you own — "if we rebuilt this from scratch, what would we recommend?" without rewriting anything. Parallel mappers ground everything in real code + git/defect history; four isolated lenses (substrate, **keep-verbatim archaeologist**, coupling, operability) propose from-scratch changes; a skeptic attacks; a synthesizer emits ONE report: keep-verbatim list, ranked load-bearing changes with the pain each retires, a **strangler-fig sequence** with effort classes, and an owner routing every recommendation to the executor skill that ships it. Detection-only — the human picks; a full-rewrite verdict routes to clean-room and stops. |
| [**gauntlet-loop**](development/gauntlet-loop/SKILL.md) | Turns any goal into **one short, paste-ready "gauntlet loop" prompt**: the agent sets a concrete quality bar (a *named, fetchable, comparable* real-world reference — Stripe's pricing page, a Julia Evans post, a shipped title's footage), splits the work into small judgeable pieces, fans out a builder and a **separate harsh critic with fresh context** per piece, compares **blind with labels stripped**, and loops until ours wins the A/B. The exit is winning the comparison, never a round count |
| [**operating-discipline**](development/operating-discipline/SKILL.md) | Frontier-agent **operating doctrine** distilled from 23k+ observed turns: ten behavioral laws (ground first, prove don't claim, never retry verbatim, fix the class, distrust success, smallest diff, state in files, batch/serialize, speak at boundaries, safety floors), a per-task operating loop, situation recipes, a tool-error recovery ladder, and a condensed **drop-in system-prompt block** for weaker-model workers. Doctrine other skills' driver prompts inherit, not a phased workflow itself. |
...
| [**greenfield-architecture**](systems-design/greenfield-architecture/SKILL.md) | **Day-zero architecture** for an empty repo: turn requirements plus a domain/data model into an architecture coding agents can safely build inside. Labels every statement `derived` / `assumed` / `chosen` (a foreign key is not a module seam; twelve entities are not twelve services), sizes via design-system, resolves each load-bearing choice through architecture-decision-loop, then emits an **architecture constitution** (`docs/architecture/`, ADRs, machine-readable seed), a minimal executable skeleton, and Guardrail candidates — and hands the bootstrap repo to guardrail-forge, which still starts at read-only discovery. Stops before product implementation. |
| [**architecture-decision-loop**](systems-design/architecture-decision-loop/SKILL.md) | One **load-bearing architecture question** at a time: frame → research (explicit constraints before docs before opinion) → at least two *genuinely different* shapes → criteria-first tradeoff matrix → fresh **adversarial skeptic** whose `blocked` findings veto acceptance → explicit human choice → ADR with rejected alternatives and revisit triggers → optional MemBerry index → Guardrail *candidates* only for deterministic consequences. Supersede, never rewrite, when direction changes. |
...
*Thirty-one skills and counting — the jar fills up over time.*
